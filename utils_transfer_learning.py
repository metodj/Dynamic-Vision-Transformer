'''
Copied from https://github.com/yitu-opensource/T2T-ViT/blob/main/utils.py
'''
...
def load_state_dict(checkpoint_path, model, use_ema=False, num_classes=1000, del_posemb=False):
    if checkpoint_path and os.path.isfile(checkpoint_path):
        ...
        # print([(x, y.shape) for x, y in state_dict.items() if 'pos_embed' in x])
        # print(model.pos_embed.shape)

        # # resize the position embedding
        # pos_embed_params = [x for x in state_dict.keys() if 'pos_embed' in x]
        # for k in pos_embed_params:
        #     old_posemb = state_dict[k]
        ...
        #         state_dict[k] = new_posemb

        # print([(x, y) for x, y in state_dict.items() if 'pos_embed' in x])
